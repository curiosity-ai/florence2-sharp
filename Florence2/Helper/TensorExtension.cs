//from: https://github.com/TensorStack-AI/OnnxStack/blob/master/OnnxStack.Core/Extensions/TensorExtension.cs
...
namespace Florence2
{
    public static class TensorExtension
    {
        ...
        private static int Multiply(ReadOnlySpan<int> dimensions)
        {
            int aggrgate = dimensions[0];
        ...
        public static DenseTensor<float> OnesFloat(ReadOnlySpan<int> dimensions)
        {
            var     resultSize = Multiply(dimensions);
            float[] arr        = new float[resultSize];
            Array.Fill(arr, 1.0f);
            var result = new DenseTensor<float>(arr, dimensions);
            return result;
        }

        public static DenseTensor<long> OnesLong(ReadOnlySpan<int> dimensions, long value = 1)
        {
            var resultSize = Multiply(dimensions);
            var arr        = new long[resultSize];
            Array.Fill(arr, value);
            var result = new DenseTensor<long>(arr, dimensions);
            return result;
        }
        ...
        /// <param name="tensor">The tensor.</param>
        /// <returns></returns>
        public static IEnumerable<DenseTensor<long>> SplitBatch(this DenseTensor<long> tensor)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                yield return new DenseTensor<long>(tensor.Buffer.Slice(start, newLength), dimensions);
            }
        }
        ...
        /// <param name="tensor">The tensor.</param>
        /// <returns></returns>
        public static IEnumerable<DenseTensor<long>> SplitBatchRemoveLeadingDimension(this DenseTensor<long> tensor)
        {
            var count      = tensor.Dimensions[0];
            var dimensions = tensor.Dimensions[1..].ToArray();
...
//        /// <param name="tensor">The tensor.</param>
//        /// <returns></returns>
//        public static IEnumerable<DenseTensor<float>> SplitBatchRemoveLeadingDimension(this DenseTensor<float> tensor)
//        {
//            var count      = tensor.Dimensions[0];
//            var dimensions = tensor.Dimensions[1..].ToArray();
//
...
//        public static void FillBatchWithValue(DenseTensor<float> tensor, int batchID, float value)
//        {
//            if (tensor.Dimensions.Length != 2) throw new ArgumentException("only tensor with 2 dimensions supported");
//
//            var start = batchID * tensor.Dimensions[1];
//            var end   = batchID * tensor.Dimensions[1] + tensor.Dimensions[1];
//
//            tensor.Buffer.Span[start..end].Fill(value);
//
//        }
        ...
        /// <returns></returns>
        /// <exception cref="System.NotImplementedException">Only axis 0,1,2 is supported</exception>
        public static DenseTensor<float> ConcatTensor(this DenseTensor<float> tensor1, DenseTensor<float> tensor2, int axis = 0)
        {
            if (tensor1 == null)
                return tensor2.ToDenseTensor();
        ...
        {
            //TODO pius: implement it with array/buffer block copy
